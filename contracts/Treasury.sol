...
contract Treasury is ContractGuard {
    ...
    modifier checkEpoch {
        ...
        epoch = epoch.add(1);
        epochSupplyContractionLeft = (getPegPrice() > pegTokenPriceCeiling) ? 0 : getPegCirculatingSupply().mul(maxSupplyContractionPercent).div(10000);
    }
    ...
    function getPegPrice() public view returns (uint256 pegPrice) {
        try IOracle(oracle).consult(pegToken, 1e18) returns (uint144 price) {
    ...
    function getPegUpdatedPrice() public view returns (uint256 _pegPrice) {
        try IOracle(oracle).twap(pegToken, 1e18) returns (uint144 price) {
            return uint256(price);
        } catch {
            revert("Treasury: failed to consult Peg price from the oracle");
        }
    ...
    function getRedeemableBonds() public view returns (uint256 _redeemableBonds) {
        uint256 _pegPrice = getPegPrice();
        if (_pegPrice > pegTokenPriceCeiling) {
            uint256 _totalPeg = IERC20(pegToken).balanceOf(address(this));
    ...
    function getBondDiscountRate() public view returns (uint256 _rate) {
        ...
        if (_pegPrice <= pegTokenPriceOne) {
            if (discountPercent == 0) {
                ...
                uint256 _bondAmount = pegTokenPriceOne.mul(1e18).div(_pegPrice); // to burn 1 PEG
    ...
    function getBondPremiumRate() public view returns (uint256 _rate) {
        uint256 _pegPrice = getPegPrice();
        if (_pegPrice > pegTokenPriceCeiling) {
            uint256 _pegPricePremiumThreshold = pegTokenPriceOne.mul(premiumThreshold).div(100);
            if (_pegPrice >= _pegPricePremiumThreshold) {
                //Price > 1.10
                uint256 _premiumAmount = _pegPrice.sub(pegTokenPriceOne).mul(premiumPercent).div(10000);
    ...
    function setPegPriceCeiling(uint256 _pegPriceCeiling) external onlyOperator {
        require(_pegPriceCeiling >= pegTokenPriceOne && _pegPriceCeiling <= pegTokenPriceOne.mul(120).div(100), "out of range"); // [$1.0, $1.2]
        pegTokenPriceCeiling = _pegPriceCeiling;
    }
    ...
    function setPremiumThreshold(uint256 _premiumThreshold) external onlyOperator {
        require(_premiumThreshold >= pegTokenPriceCeiling, "_premiumThreshold exceeds pegPriceCeiling");
    ...
    function getPegCirculatingSupply() public view returns (uint256) {
        IERC20 pegErc20 = IERC20(pegToken);
    ...
    function buyBonds(uint256 _pegAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        require(
            pegPrice < pegTokenPriceOne, // price < $1
        ...
        IBasisAsset(pegToken).burnFrom(msg.sender, _pegAmount);
    ...
    function redeemBonds(uint256 _bondAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        require(pegPrice == targetPrice, "Treasury: Peg price moved");
        require(
            pegPrice > pegTokenPriceCeiling, // price > $1.01
            "Treasury: pegPrice not eligible for bond purchase"
        );
        ...
        require(IERC20(pegToken).balanceOf(address(this)) >= _pegAmount, "Treasury: treasury has no more budget");
        ...
        IERC20(pegToken).safeTransfer(msg.sender, _pegAmount);
    ...
    function _sendToMasonry(uint256 _amount) internal {
        ...
        IERC20(pegToken).safeApprove(masonry, 0);
        IERC20(pegToken).safeApprove(masonry, _amount);
