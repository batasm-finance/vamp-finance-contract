...
contract ShareWrapper {
    ...
    function stakeForAddress(address to, uint256 amount) public virtual {
        _totalSupply = _totalSupply.add(amount);
...
contract Masonry is ShareWrapper, ContractGuard {
    ...
    event StakedForAddress(address indexed user, uint256 amount);
    ...
    function initialize(
        ...
        rewardLockupEpochs = 6; // Lock for 6 epochs (36h) before release claimReward
    ...
    function stakeForAddress(address to, uint amount) public override onlyOneBlock updateReward(to) {
        // Only use this before Masonry begins(Epoch 0).
        require(amount > 0, "Masonry: Cannot stake 0");
        require(treasury.epoch() == 0, "Masonry: Epoch has to be 0!");
        super.stakeForAddress(to, amount);
        masons[to].epochTimerStart = treasury.epoch(); // reset timer
        emit StakedForAddress(to, amount);
    }
